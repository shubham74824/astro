...
authRoutes.post("/send_otp", async (req, res) => {
  ...
  try {
    ...
    if (user) {
      ...
      const newResponse = await newUser.save();
...
authRoutes.post("/verify_otp", async (req, res) => {
  ...
  // Generate JWT token
  const token = jwt.sign({ id: target._id, number }, secretKey, {
    expiresIn: "7h",
  });
...
authRoutes.post("/register_astro", async (req, res) => {
  const { fullName, number, gender, email, experience, city, shortBio } =
    req.body;
  ...
  const existingAstro = await Astro.findOne({
    $or: [{ number }, { email }],
  }).exec();
  ...
  if (existingAstro && existingAstro.isProfileCompleted) {
    return res
      .status(500)
      .json({ message: "Number or email is already registered" });
  }
